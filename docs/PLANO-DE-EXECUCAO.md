...
- [ ] **7. Corrigir o binding de parâmetros SQL ($N)** *(P)* — **crítico, próximo PR.** `executeSql` e `toLibsqlStatement` (`db.ts`) trocam `$1`, `$2`... por `?` na ordem em que aparecem no texto, ignorando o número capturado. Um placeholder repetido consome um argumento a menos e escreve na coluna errada sem falhar. Usar `params[Number(number) - 1]` nas duas funções, com o mesmo comportamento em `executeSql` e `executeTransaction`. Testes: ordem normal, fora de ordem, `$1` repetido, número inexistente, argumento faltando; varrer o repositório por consultas que dependam do comportamento antigo.
...
2. **Agora, nesta ordem — nenhum algoritmo novo antes disso:**
   1. **7** — binding de `$N` (afeta toda query com parâmetros; risco cresce a cada endpoint novo).
...
3. **Depois, segurança de custo baixo e risco alto:** 17 (✅ PR #28, aguardando merge) → 18 → 16 → 13.
