...
**370 testes** em `main`, contra 160 quando este plano foi escrito.
...
- [ ] **13. Argon2id, rate limiting e anti-enumeração** *(M)* — **dividido em dois, porque as duas metades têm risco muito diferente.**

  **13a — rate limiting e anti-enumeração:** ✅ [PR #40](https://github.com/LuizArgenta/open-alpha/pull/40). Não muda hash guardado nem adiciona dependência. Contador no banco (`auth_attempts`, migração 006), limite por e-mail e por IP, só falhas contam e um login bem-sucedido zera o orçamento — quem erra a senha quatro vezes e lembra na quinta não fica a quatro falhas de um bloqueio. **O buraco de enumeração real não era a mensagem, era o tempo:** e-mail inexistente retornava *antes* de qualquer comparação, ~1 ms contra os ~100 ms de uma conta real — oráculo confiável para saber se um endereço tem conta aqui, o que numa plataforma de contas de crianças não é detalhe. Agora os dois caminhos comparam um bcrypt de verdade.

  **13b — Argon2id:** ainda aberto. Exige dependência nova (`bcryptjs` hoje) e migração transparente dos hashes bcrypt existentes no login, já que ninguém tem a senha em claro para reprocessar. Vale como PR próprio.

  **Limite honesto do que 13a entrega:** o signup **continua revelando** se um endereço já tem conta — ele precisa responder "e-mail já cadastrado" para o formulário funcionar, e fechar isso de verdade exige fluxo de verificação por e-mail que este deploy não tem. O rate limiting por IP mitiga varrer uma lista, não impede checar um endereço. E o limite por IP é mais fraco que o por e-mail: `x-forwarded-for` é cabeçalho de requisição, então atrás de um proxy que o reescreve (Vercel) ele vale, e rodando direto ele é conselho. O limite por e-mail é o que segura de verdade, porque o atacante não consegue evitar nomear a conta que quer.

  **Achado de privacidade no caminho:** `demo/chat.ts` hasheava IP com o hash de string de 32 bits do Java, com um comentário afirmando que era "o suficiente para não guardar IPs reais". Não era: o espaço IPv4 inteiro são quatro bilhões de tentativas contra 32 bits, ou seja, segundos — e as colisões ainda faziam dele um mau limitador, com visitantes sem relação dividindo o mesmo balde. Passou a usar o `rateLimitKey` compartilhado: SHA-256 com o segredo do deploy misturado.
...
- [x] **16. Varredura de IDOR/BOLA** *(M)* — [PR #39](https://github.com/LuizArgenta/open-alpha/pull/39). **A varredura não achou nenhuma falha.** Todo endpoint que aceita identificador vindo do navegador já confere posse: `interests` apaga com `AND user_id`, `tutor/chat` e `coach/chat` buscam a sessão com `AND user_id`, `coach/chat` exige vínculo com `linked_at IS NOT NULL`, as quatro rotas `parent/children/[childId]/*` passam pelo `requireLinkedChild`, e prova, nivelamento e submissão comparam `student_id` com o do token. Vale dizer isso sem enfeite: **este item entregou verificação, não correção.**

  O que faltava era exatamente isso — verificação. O critério deste item era "endpoint sem teste de acesso cruzado conta como não varrido", e prova, nivelamento, timeline e override já tinham o seu. Os 15 testes novos cobrem o que faltava: progresso (resumo, atividade recente, fila de revisão, timeback, mapa), sessões de tutor, interesses, coach e as rotas de analytics/progresso do responsável — incluindo o caso do convite **enviado mas não aceito**, que não pode já dar acesso ao registro da criança. Confirmei que os testes mordem, sabotando de propósito o `requireLinkedChild` e o escopo do `DELETE` de interesses: 5 falham, e voltam a passar com o código restaurado.

  **Endpoints sem nada a varrer**, porque não recebem identificador de usuário do cliente e derivam tudo do token: `progress/contest`, `progress/events`, `progress/summary`, `progress/timeback`, `progress/review`, `progress/activity/recent`, `progress/map/[subject]`, `tutor/next/[subject]`, `tutor/concepts/[subject]`, `coach/children`, `parent/link`, `parent/generate-invite`, `parent/create-child`. Registrado para que "varrido" signifique algo verificável em vez de uma sensação.
...
3. **Depois, segurança de custo baixo e risco alto:** 17 (✅ PR #28) → 18 (parcial, PR #38 — o resto é configuração do repositório) → 16 (✅ PR #39) → 13a (✅ PR #40) → 13b (Argon2id).
