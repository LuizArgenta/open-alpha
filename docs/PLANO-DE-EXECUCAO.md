...
**Em voo:** [#32](https://github.com/LuizArgenta/open-alpha/pull/32) (draft) traz o banco de itens — item 19, e de quebra o núcleo do item 10. CI verde, 226 testes na branch, mas **conflita com o #31 em `db.ts`** e precisa de rebase antes de sair de draft. Auditado em detalhe no próprio PR.

**221 testes** em `main`, contra 160 quando este plano foi escrito.
...
- [ ] **9. Tornar a finalização da tentativa concorrente-segura e idempotente** *(M)* — completa o item 2. Preferência: tabela `assessment_attempt_finalizations` com `attempt_id` como chave primária, `INSERT` dela como primeira instrução da transação — uma segunda finalização viola a chave e reverte tudo. **A alternativa (`UPDATE ... WHERE finished_at IS NULL RETURNING id`, seguir só se mudou uma linha) exige ler dentro da transação, o que a API atual de `executeTransaction` não permite** — ver item 11. **Também exige schema:** `xp_awards` não tem `attempt_id` hoje (só `student_id, subject, concept_id, amount, reason`), então "um único XP por tentativa" precisa da coluna mais índice único.

  Casos de aceite, além do óbvio: (a) duas submissões via `Promise.all` → um sucesso, um 409, **um** XP, `attempts` +1 e um só conjunto de decisões; (b) três ou mais simultâneas, porque duas podem passar por sorte de escalonamento; (c) submissão concorrente com `expireStaleAttempts` (o aluno abre outra prova em outra aba) → a tentativa não pode ficar expirada **e** pontuada; (d) submissão concorrente com uma resposta ainda em voo em `answer.ts` → a nota não conta resposta que chegou depois do fechamento; (e) reenvio após 409 não altera nada; (f) falha injetada na última instrução → nenhuma escrita sobrevive e a tentativa continua re-submissível; (g) tentativas concorrentes do mesmo aluno em conceitos **diferentes** não podem se bloquear — o lock é por tentativa, não por aluno; (h) os dois ramos de `progress` (INSERT e UPDATE) precisam do mesmo teste — o bug do `$4/$5/$6` achado no #29 estava exatamente no ramo do UPDATE. **O teste tem de falhar contra o código de hoje:** teste de concorrência que passa antes do fix está testando o escalonador, não a correção.
- [ ] **10. Versionar itens autorados como snapshots imutáveis** *(M)* — **majoritariamente entregue pelo [#32](https://github.com/LuizArgenta/open-alpha/pull/32)**, que casa `storeItem` por `content_hash` e versiona cada snapshot: editar a questão mantendo o `authoredId` passa a criar item novo, e o aluno deixa de ser corrigido por gabarito velho. **O que sobra, depois que o #32 mesclar:** o hash é uma *projeção manual* — `snapshotItem()` enumera 11 campos à mão, e um campo novo em `AttemptQuestion` que ninguém espelhe ali faz dois itens diferentes colidirem no mesmo hash, ressuscitando este bug em silêncio. Falta um teste que quebre quando `AttemptQuestion` ganhar campo fora do hash. A garantia "byte a byte" hoje vem da *construção* do hash, não de ler o snapshot de volta do banco.
- [ ] **11. Tornar `openAttempt` transacional** *(M, era P)* — hoje grava itens, depois a tentativa, depois os vínculos em `executeSql` separados (`assessment.ts`); uma falha no meio deixa itens órfãos ou tentativa incompleta. **O #32 alarga essa janela**: `storeItem` passou a fazer transações internas e até 4 retries por item, então há mais estado parcial a perder. Subiu de *(P)* para *(M)* por causa disso. **Pré-requisito não óbvio:** exige o `id` da tentativa *no meio* da transação para inserir os vínculos, e a API atual de `executeTransaction` recebe lista preparada de statements — não dá para ler no meio. Ou o `executeTransaction` ganha uma variante com callback, ou este item não sai. Esse mesmo pré-requisito vale para a alternativa do item 9, então vale resolvê-lo uma vez, antes dos dois.
- [ ] **12. Criar migrações versionadas e parar de engolir erros** *(M)* — `initializeSchema` roda cada `ALTER TABLE` num `try/catch` que descarta qualquer exceção como "coluna já existe" — inclusive erro de permissão, conexão, corrupção ou constraint. Criar `_schema_migrations` (id, checksum, data), ignorar só os códigos de erro esperados, falhar o health check se uma migração necessária não terminar. **Escopo concreto:** já existem **três** migrações deliberadas fora do loop que engole erros, escritas por três sessões independentes com três estilos de guarda diferentes — `migrateGeneratedLessonsToPerLanguage` (checa `PRAGMA table_info`), `ensureAssessmentResponsesUniqueConstraint` (checa `PRAGMA index_list`, do #31) e `migrateAssessmentItemBank` (checa PRAGMA e faz backfill, do #32). Este item é **consolidar as três num registry versionado**, não adicionar uma quarta. Absorve também o diff genérico de schema entre banco novo e migrado que o item 8 deixou fora do escopo.
...
- [ ] **19. Banco de itens** *(G)* — pool por habilidade em vez de 5 questões fixas, com dificuldade, distratores ligados a erros conhecidos e versão do item registrada em cada tentativa. **Destrava 20, 21 e 23.** **[PR #32](https://github.com/LuizArgenta/open-alpha/pull/32) aberto (draft)** — implementado e com CI verde, aguardando rebase sobre o #31. Chegou fora da ordem desta lista: a sequência dizia "só então 19 em diante" e o trabalho aconteceu antes. Registrado assim em vez de reescrever a história.
...
2. **Agora, nesta ordem — nenhum algoritmo novo antes disso:**
   ...
   3. **#32 sai de draft** — rebase sobre o #31 e re-rodar a suíte. Não é "furar a fila": o trabalho já existe, está verde, e mantê-lo aberto só aumenta a chance de conflitar de novo. Fecha o item 19 e a maior parte do 10.
   4. **API de transação com callback** — pré-requisito compartilhado dos itens 9 e 11, hoje invisível no plano. O `executeTransaction` recebe lista preparada de statements e não deixa ler no meio; sem isso o item 11 não sai e o item 9 fica preso à variante da tabela de finalizações.
   5. **11** — `openAttempt` transacional. Antes do 9 porque o #32 alargou a janela dele.
   6. **9** — finalização da tentativa concorrente-segura e idempotente.
   7. **10** — o que sobrar depois do #32: teste que impeça o hash de sair de sincronia com `AttemptQuestion`.
   8. **12** — migrações versionadas, consolidando as três que já existem.
