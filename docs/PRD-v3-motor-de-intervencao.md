...
| **0.1c** ✅ | **Degradar em vez de recusar, e contar** — [PR #55](https://github.com/LuizArgenta/open-alpha/pull/55) | O 0.1a tratava metadado faltando como erro fatal: mapa de códigos incompleto derrubava a prova inteira com 502. Trocar a sessão de um aluno por qualidade de dado é a troca errada num produto de aprendizagem. Agora: gera; se veio incompleto, **tenta uma segunda vez**; se persistir, serve o item com o mapa duvidoso **descartado** — meio preenchido mente ao diagnóstico, ausente é honesto — e registra a omissão no payload do `quiz_start`. Item **impossível de responder** continua recusando: não há versão dele que sirva a um aluno. Critério: `GET /api/health/generation` responde com que frequência a geração omite o metadado. Sem esse número, servir degradado seria a quinta ponta solta do projeto — construída, e ninguém olhando. |
...
| **1.3** ✅ | **`Intervention` + `InterventionRun`** — [PR #57](https://github.com/LuizArgenta/open-alpha/pull/57) | As quatro ações de remediação que o motor já escolhe viram catálogo semeado por migração, e cada escolha abre um `intervention_run` com `reason`, `evidence` e **`expected_outcome` gravado antes do resultado** — a próxima tentativa no mesmo conceito julga a previsão (`met` / `not_met` / `inconclusive`), e o que o aluno nunca retomou fecha como `abandoned` na varredura que já roda ao abrir prova. O guardrail da seção 6.2 virou teste: ação de professora, vídeo externo, tutoria de IA e atividade entre pares passam **pelo mesmo caminho** do motor — nenhum ramo lê `source`. Resposta ao aluno inalterada; `nextAction` é o 1.4. |
| **1.4** ✅ | **Decisão retorna `nextAction`** — [PR #58](https://github.com/LuizArgenta/open-alpha/pull/58) | Os dois pontos onde o motor decide (`next/{subject}` e `submit`) devolvem **ação**, com `reason`, `conceptId` e `policyVersion`; `concept` e `remediation` permanecem por compatibilidade. Cinco tipos, e **cada um tem produtor** — não os dez que um motor mais rico teria. Duas diferenças reais: avançar e **voltar a um pré-requisito** deixaram de ser a mesma resposta, e `simpler_explanation`, que dizia ao aluno "vou explicar de outro jeito" e **não oferecia botão nenhum** (porque a tela inferia intenção de qual campo vinha preenchido), agora abre a lição. O PRD escrevia `interventionId`; devolvo **`interventionRunId`** — o id era ambíguo entre "que tipo de intervenção" e "a instância deste aluno", e é a segunda que o cliente precisa. |
| **1.5** 🟡 | **Resultados de intervenção** — [PR #59](https://github.com/LuizArgenta/open-alpha/pull/59) | **Linha do tempo: feito.** A aposta e o veredito existiam desde o 1.3 e viviam só no banco; agora um adulto vê o que foi oferecido, *o que se esperava disso*, e como terminou. Duas entradas por run, não uma: o intervalo entre "ofereceu" e "concluiu que não funcionou" é justamente o que se está perguntando, e uma linha só com veredito o esconde. **Stream: adiado, e por quê.** Adicionar `intervention_started`/`intervention_completed` a `learning_events` hoje seria escrever numa ponta sem leitor na outra — a linha do tempo lê `intervention_runs`, que é onde a previsão e o veredito realmente moram, e é o que o próprio non-goal do §6.5 manda (as tabelas operacionais seguem sendo a fonte). O consumidor real do stream aparece na **onda 2**, com exportação e análise; os tipos de evento entram junto com ele. |
| **1.6** ✅ | **Modelo de procedência** — [PR #62](https://github.com/LuizArgenta/open-alpha/pull/62) | Cinco colunas em conceito e intervenção, com prefixo `content_` porque `interventions.source` já significa outra coisa — **quem entrega** a intervenção, não de onde veio o conteúdo. A guarda fica no **publish**: é o último momento barato para perguntar de onde veio e sob que licença, e recusa em lista, não na primeira falha. Quatro origens, cada uma com sentido hoje: `original` (os 141 semeados), `contributed`, `generated` e `external` — esta última exigindo URL, licença e atribuição. **Share-alike recusado por ora**, como o PRD manda: alcança o que a plataforma constrói em cima, não só o conteúdo. É uma constante e levantar é decisão de uma linha. |
| **1.7** ✅ | **Adaptador de conteúdo aberto** — [PR #63](https://github.com/LuizArgenta/open-alpha/pull/63) | O corpus entra como **bundle** — arquivo no formato deste projeto — e não por chamada de API. Não é atalho: corpora se movem em dumps e exportações, não em consultas ao vivo em volume de importação. Converter o dump de terceiro em bundle é um conversor, escrito quando houver dump. A alternativa era construir o importador contra um palpite de memória do formato de outro serviço, produzindo algo que **parece pronto e nunca encontrou aquilo que diz ler**. Importa como **rascunho**, sempre: a guarda de procedência mora no publish, e um import que passasse por ela derrubaria o 1.6. **Tudo ou nada**, porque importação parcial de grafo deixa pré-requisitos apontando para conceitos recusados. Atribuição chega **ao aluno**, no rodapé da lição — e só sobre o material que ela de fato cobre: se a lição exibida foi gerada, o crédito não aparece, porque citação falsa é pior que nenhuma. |
